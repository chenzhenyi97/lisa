(function( window, undefined ) {
    ...
    cjQuery.extend({
        ...
        isArray : function(sele){
            if(cjQuery.isObject(sele) &&
                !cjQuery.isWindow(sele) &&
                "length" in sele){
                return true;
            }
            return false;
        ...
        clone: function (deep) {
            ...
            if(deep){
                ...
                this.each(function (key, ele) {
                    ...
                    // 遍历元素中的eventsCache对象
                    cjQuery.each(ele.eventsCache, function (name, array) {
                        // 遍历事件对应的数组
                        cjQuery.each(array, function (index, method) {
                            // 给复制的元素添加事件
                            $(temp).on(name, method);
                        });
                    });
    ...
    cjQuery.prototype.extend({
        next: function (sele) {
            ...
            if(arguments.length === 0){
                // 返回所有找到的
                this.each(function (key, value) {
                    var temp = cjQuery.get_nextsibling(value);
                    if(temp != null){
                        res.push(temp);
                    }
                });
        ...
        prev: function (sele) {
            ...
            if(arguments.length === 0){
                this.each(function (key, value) {
                    var temp = cjQuery.get_previoussibling(value);
    ...
    cjQuery.prototype.extend({
        ...
        css: function (attr, value) {
            ...
            if(cjQuery.isString(attr)){
                // 判断是一个字符串还是两个字符串
                if(arguments.length === 1){
                    return cjQuery.getStyle(this[0], attr);
                }else{
                    this.each(function (key, ele) {
    ...
    cjQuery.prototype.extend({
        on: function (name, callBack) {
            ...
            this.each(function (key, ele) {
                ...
                if(!ele.eventsCache[name]){
                    ...
                    // 5.添加对应类型的事件
                    cjQuery.addEvent(ele, name, function () {
                        cjQuery.each(ele.eventsCache[name], function (k, method) {
                            method.call(ele);
                        });
                    });
                }else{
        ...
        off: function (name, callBack) {
            ...
            // 3.判断是否传入了两个参数
                ...
                this.each(function (key, ele) {
                    cjQuery.each(ele.eventsCache[name], function (index, method) {
